...
if __name__ == '__main__':
    ...
    mp = bot.mps()
    #producer = KafkaProducer(bootstrap_servers='10.61.120.136:9092',value_serializer=lambda v: json.dumps(v).encode('utf-8'))
    @bot.register(mp, except_self=False)
    def waibufun(ms):
        print_mes(ms)
        #print ms.raw
	#print type(kk)
	mp = bot.mps(update=True)
        #producer.send('weixin',kk)
